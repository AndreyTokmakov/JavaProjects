...
public class Synchronized_Method_Test {
	...
	protected void Test_NotSynched() throws InterruptedException {
		...
		System.out.println("Not Synchronized test: result = " + resource.getCounter());
	}
	
	protected void Test() throws InterruptedException {	
		SharedResouce_Sync resource = new SharedResouce_Sync(); // Synchronized resource
		ArrayList<Thread> threads = new ArrayList<Thread>();
		for (int i = 0; i < 10; i++) {
			threads.add(new Thread(new Worker2(resource)));
			threads.get(threads.size() -1).start();
		...
		System.out.println("Synchronized test    : result = " + resource.getCounter());
	}
	
	public static void main(String[] args) throws InterruptedException {
		Synchronized_Method_Test tests = new Synchronized_Method_Test();
		tests.Test_NotSynched();
		tests.Test();
	}
}
