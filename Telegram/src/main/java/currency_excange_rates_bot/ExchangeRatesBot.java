...
public class ExchangeRatesBot extends AndTokmBotBase {
    ...
    public ExchangeRatesBot() {
        LOG.debug("Starting the ExchangeRatesBot");
    }
    ...
    private void aedCommand(Long chatId) {
        String formattedText;
        try {
            String usd = exchangeRatesService.getAEDExchangeRate();
            String text = "Курс Дихрам на %s составляет %s рублей";
            formattedText = String.format(text, LocalDate.now(), usd);
        } catch (ServiceException e) {
            LOG.error("Ошибка получения курса Дихрам", e);
            formattedText = "Не удалось получить текущий курс Дихрам. Попробуйте позже.";
        }
        sendMessage(chatId, formattedText);
    }
